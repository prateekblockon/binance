...
$.getJSON('https://www.highcharts.com/samples/data/from-sql.php?callback=?', function (data) {
    ...
    Highcharts.stockChart('container', {
			...
			<input type="hidden" id="subcurr" value="<?php echo $subcurr; ?>">
				...
				<div class="main-body">
					...
					<div class="main-left">
						...
						<div class="depths" ng-class="{'iswrong':depthWrong}">
							...
							<div class="depths-wrap scrollStyle" id="askScrollBox" ng-class="{'cur':curDepthType==3,'other':curDepthType==2}">
							...
							<div class="depths-wrap scrollStyle" id="bidScrollBox" ng-class="{'cur':curDepthType==2,'other':curDepthType==3}">
					...
					<!-- 下单form -->
					<div class="orderforms f-cb" ng-init="orderType={'v':'limit'}">
						<div class="orderform-type">
							<span ng-class="{true:'cur',false:''}[orderType.v=='limit']" ng-click="orderType.v='limit'">{{'LimitOrder' | T}}
							</span>
						</div>
						<!-- 限价委托 -->
						<div class="orderform-main f-cb" >
							<div class="f-fl">
								<div class="orderforms-hd">
									<div>
										<label>BUY  <?php echo $subcurr; ?></label>
										<span class="f-fr" id="maincurrbalance"></span> <span class="f-fr" >Available Balance: </span>
								...
								<form name="buyForm"  novalidate autocomplete="off">
									...
									<div class="orderforms-inputs">
										...
										<div class="field f-cb">
											...
											<div class="iptwrap leftBig">
												<input type="text" name="buyPrice" id="buyPrice"  ng-keyup="checkinput($event)" onpaste="return false"   ng-model="buy_order.price"  required/>
										...
										<div class="field f-cb">
											...
											<div class="iptwrap" >
												<input type="text" name="buyQuanity" id="buyQuanity" ng-keyup="checkinput($event,currentProduct.qtyTick)"  ng-blur="maxQuantity=false;" onpaste="return false" ng-focus="maxQuantity=true;hideerror($event,'buyform-error')" ng-model="buy_order.quantity"  required />
										...
										<div class="field f-cb">
											<label>{{'total'|T}}: </label>
											<div class="iptwrap">
												<input class="readonly" type="text" value="{{(buy_order.quantity*buy_order.price || 0 )| number:8}}" id="bid_vol" readonly/>
												<span class="unit"><?php echo $maincurr; ?></span>
											</div>
										</div>
									</div>
									...
									<?php if($this->session->userdata('_id')=='')
				                        { ?>
										<div  class="logout"><a ng-href="/login.html?callback={{urlPath}}">{{'login'|T}}</a> or <a href="/register.html">{{'register'|T}}</a> {{"to-trade"|T}}</div>
										<? } else {?>

										<input type="submit" onclick="but_data();" value="{{'Buy' | T}} {{currentProduct.baseAsset}} <?php echo $subcurr; ?>" class="btn btn-block btn-buy" />
										<?php } ?>
									</div>
								</form>
							</div>
							<div class="f-fr">
								<div class="orderforms-hd orderforms-hd-magenta">
									<div>
										<label>Sell  <?php echo $subcurr; ?></label>
										<span class="f-fr" id="subcurrbalance"></span> <span class="f-fr" >Available Balance: </span>
								...
								<form name="sellForm"  autocomplete="off">
									...
									<div class="orderforms-inputs">
										...
										<div class="field f-cb">
											<label>{{'price'|T}}: </label>
											<div class="iptwrap leftBig">
												<input type="text" name="sellPrice" id="sellPrice" ng-keyup="checkinput($event,currentProduct.priceTick)" onpaste="return false" ng-focus="hideerror($event,'sellform-error')"   ng-model="sell_order.sell_price"/>
												<span class="legalMoney" ng-if="sell_order.sell_price && cur_lang=='cn'">{{sell_order.sell_price*transToUSDT[quoteAsset]*cnyusdRate | currency:'￥'}}</span>
										...
										<div class="field f-cb">
											<label>{{'Amount'|T}}: </label>
											<div class="iptwrap" ng-init="sell_maxQuantity=false">
												<input type="text" name="sellQuanity" id="sellQuanity" ng-keyup="checkinput($event,currentProduct.qtyTick)"  ng-blur="sell_maxQuantity=false;" onpaste="return false" ng-focus="sell_maxQuantity=true;hideerror($event,'sellform-error')"   ng-model="sell_order.sell_quantity" />
										...
										<div class="field f-cb">
											<label>{{'total'|T}}: </label>
											<div class="iptwrap">
												<input class="readonly" type="text" id="ask_vol" value="{{sell_order.sell_quantity*sell_order.sell_price || 0 | number:8}}" readonly/>
												<span class="unit"><?php echo $maincurr; ?></span>
											</div>
										</div>
									</div>
									...
									<?php if($this->session->userdata('_id')=='')
				                        { ?>
									    <div class="logout"><a href="/login.html?callback={{urlPath}}">{{"login"|T}}</a> or <a href="/register.html">{{"register"|T}}</a> {{'to-trade'|T}}</div>
									    <? } else { ?>

										<input type="submit" onclick="sell_data();" value="{{'Sell' | T}} {{currentProduct.baseAsset}} <?php echo $subcurr; ?>"  class="btn btn-block btn-sell" />
										<?php } ?>
									</div>
					...
					<div class="main-aside" style="top:-58px;">
						<div class="depths" style="margin-bottom:-2px;">
							<div class="depthstitle f-cb">
								<ul class="tab-box" ng-init="curMarket=getQuoteAsset">
									<li class="f-fl" id="defaultStart" ><span class="iconfont icon-xingxing activeTabDefault">{{'Favorites' | T}}</span></li>
									
									<li class="f-fr  f-nomargin" ng-click="curMarket='USDT';isPro='';changeMarket(curMarket)" ng-class="{true:'active',false:''}[curMarket=='USDT']">USDT</li>
									<li class="f-fr" ng-click="curMarket='BNB';isPro='';changeMarket(curMarket)" ng-class="{true:'active',false:''}[curMarket=='BNB']">BNB</li>
									<li class="f-fr" ng-click="curMarket='ETH';isPro='';changeMarket(curMarket)" ng-class="{true:'active',false:''}[curMarket=='ETH']">ETH</li>
									<li class="f-fr" ng-click="curMarket='BTC';isPro='';changeMarket(curMarket)" ng-class="{true:'active',false:''}[curMarket=='BTC']">BCH</li>
								</ul>
						...
						<div class="item-title" ng-init="tradesType={type:'marketTrades'}">
							<h3>Trade History</h3>
							<div class="item-title-btns" >
								<span ng-class="{'cur':tradesType.type=='marketTrades'}" ng-click="tradesType.type='marketTrades'">Market</span>
								
							</div>
							
						</div>
						<div class="item-con" id="tradeHistory" style="margin-top:1px;">
						<div style="height:302px;overflow-y:auto;" class="scrollStyle" ng-if="tradesType.type=='marketTrades'">
				...
				<div class="userRecords f-cb" >
					<div class="userRecords-title">
						<span class="cur">{{'My 24h Order History'|T}}</span>
		...
		<div id="pop-user" ng-if="showPopUser" class="pop-user" ng-click="stopBubble($event)">
			...
			<div class="right-pop">
				<h2>User Info</h2>
				<!-- <img class="close" ng-click="showPop()" src="/exchange/resources/img/close.png"/> -->
				<div class="user-info">
					<!-- <img id="pop-close" ng-src="{{personalInfo.pic}}" src="/exchange/resources/img/touxiang.png"> -->
					<div class="right">
...
<div id="openModal" class="modalDialog">
		...
		<div class="safeTip" >
			...
			<div class="decp">
			<p>{{"We strongly recommend you to enable 2FA on your account !"|T}}</p>
		    <p>{{"Please choose how you wish to receive 2FA code:"|T}}</p>
			</div>
			<div class="yzChoose">
			<a href="<?php echo tfauth_url(); ?>tfauth" class="btgoogle" ><span class="imgicon img-google1"></span><p >{{"Google Authentication"|T}}</p></a>
			<a ng-show="user.mobileNo!=null" onclick="layer.closeAll()"   class="btphone"><span class="imgicon img-phone"></span><p>{{"SMS Authentication"|T}}</p></a>
			<a ng-show="user.mobileNo==null" href="<?php echo tfauth_url(); ?>smsotp"  class="btphone"><span class="imgicon img-phone"></span><p>{{"SMS Authentication"|T}}</p></a>
			
			</div>
			<div class="smalltip"><span>{{"I understand the risks for not enabling 2FA"|T}}</span><a style="    font-size: 12px;padding: 8px 12px;" title="Close" class="close" class="btn btn-orange">{{"Skip for now"|T}}</a></div>
